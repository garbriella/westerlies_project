...
with tabs[0]:
    ...
    st.markdown(
        """
        <div class="note-card">위도에 따른 태양에너지 수수 차이가 적도와 극 사이의 기온 경도를 만들고, 이것이 열풍 관계를 통해
        고도가 높아질수록 편서풍이 강해지는 현상과 연결됨을 학습하였다. 그러나 마찰이 크게 작용하는 지표에서도 중위도의 장기 평균
        바람이 서풍 성분을 유지하는 이유는 남북 기온 차만으로 충분히 설명하기 어렵다고 판단하였다. 이에 NOAA의 고층·지상 관측과
        재분석 자료를 이용하여 상층 편서풍의 연직 강화와 지상 서풍대의 유지 과정이 어떻게 다르고 연결되는지 분석한다.</div>
        """,
        unsafe_allow_html=True,
    )
    st.subheader("연구 질문")
